...
2. **Add the 13 required Skill Metadata Protocol fields** — `schema_version: 5`, `version`, `type`, `category`, `scope`, `owner`, `freshness`, `drift_check`, `eval_artifacts`, `eval_state`, `routing_eval`, plus your existing `name` and `description`. The template inline-comments each field. `category` is a closed enum of 6 values — see SKILL_METADATA_PROTOCOL.md § Classification.
